...
# Make tasks
all: test-once bundle-handsontable
...
bundle-handsontable:
	# Build handsontable bundle
	mkdir -p build
	NODE_PATH=$(NODE_PATH) $(GRUNT) bundle-handsontable $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-handsontable/js/Gruntfile.js
...
ifeq ($(NODE_VERSION_LT_011),true)
	...
	NODE_PATH=$(NODE_PATH) $(GRUNT) sed:bootstrap $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-handsontable/js/Gruntfile.js

jshint:
	# Codecheck
	NODE_PATH=$(NODE_PATH) $(GRUNT) jshint $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-handsontable/js/Gruntfile.js

watch:
	# Run grunt and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) watch $(DEBUG) $(VERBOSE) --gruntfile=src/mockup-handsontable/js/Gruntfile.js

test:
	# Run tests headless via PhantomJS and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-handsontable/js/Gruntfile.js

test-once:
	# Run the tests headless with PhantomJS only once and exit.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_once $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-handsontable/js/Gruntfile.js

test-dev:
	# Run the tests for Chromium and watch for changes.
	NODE_PATH=$(NODE_PATH) $(GRUNT) test_dev $(DEBUG) $(VERBOSE) --pattern=$(pattern) --gruntfile=src/mockup-handsontable/js/Gruntfile.js
...
.PHONY: all plone plone4 bundle-handsontable
